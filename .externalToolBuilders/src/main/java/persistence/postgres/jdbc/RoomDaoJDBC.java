...
public class RoomDaoJDBC implements RoomDao {
	...
	/*
	 * @Override public ArrayList<Room> retrieve(Integer nRoom, Integer maxRoom) {
	 * 
	 * try(Connection connection = this.dataSource.getConnection()) {
	 * 
	 * ArrayList<Room> result = null; PreparedStatement maxpost =
	 * connection.prepareStatement("SELECT max(id) AS max FROM room"); ResultSet
	 * rsMax = maxpost.executeQuery(); rsMax.next(); Integer idMax = (int)
	 * rsMax.getLong("max"); if ((maxRoom + nRoom) == idMax) return null;
	 * 
	 * Integer from = idMax - maxRoom; //maxPost + nPost - maxPost => nPost Integer
	 * to = from - nRoom; //nPost - nPost => 0
	 * 
	 * PreparedStatement create =
	 * connection.prepareStatement("SELECT * FROM room WHERE id <= ? and id > ?");
	 ...
	 * 
	 * Room room = new Room(); // Creating a user object to fill with // user data
	 * 
	 * room.setId(rs.getInt("id")); room.setTipo(rs.getString("tipo"));
	 ...
	 * room.setPrezzo(rs.getInt("prezzo")); room.setImg(rs.getString("img"));
	 * 
	 * // Add the retrived user to the list result.add(room); }
	 * 
	 * if (result != null) {
	 * 
	 * ArrayList<Room> orderedRoom = new ArrayList<Room>(); for (int i =
	 * result.size() - 1; i >= 0; i--) { orderedRoom.add(result.get(i)); } return
	 * orderedRoom; } return null;
	 * 
...
*/
	 ...
	 * 
	 * String query =
	 * "SELECT r.* FROM room AS r, prenotazioni AS pr WHERE pr.id_room = r.idcamera AND (pr.checkin NOT BETWEEN SYMMETRIC ? AND ?) AND (pr.checkout NOT BETWEEN SYMMETRIC ? AND ?)"
	 * ; List<Room> ret = null;
	 * 
	 * try(JDBCQueryHandler handler = new JDBCQueryHandler(query)) {
	 * 
	 * handler.getStatement().setDate(1, checkin); handler.getStatement().setDate(2,
	 * checkout); handler.getStatement().setDate(3, checkin);
	 * handler.getStatement().setDate(4, checkout); handler.executeQuery();
	 * 
	 * if(handler.existsResultSet()) { ret = new ArrayList<Room>(); ResultSet result
	 * = handler.getResultSet();
	 * 
	 * Room room = null; while (result.next()) { room = new Room();
	 * room.setId(result.getInt("idcamera"));
	 * room.setTipo(result.getString("tipo"));
	 * room.setDescrizione(result.getString("descrizione"));
	 * room.setMaxpersone(result.getInt("maxpersone"));
	 * room.setOccupata(result.getBoolean("occupata"));
	 * room.setPrezzo(result.getInt("prezzo"));
	 * room.setImg(result.getString("img")); ret.add(room); } }
	 * 
